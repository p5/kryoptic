...
extern "C" fn fn_initialize(_init_args: CK_VOID_PTR) -> CK_RV {
    ...
    if wstate.is_initialized() {
        ...
        /* Apply entropy source configuration before FIPS initialization.
         * This must happen before wstate.initialize() because that calls
         * ossl::fips::init() which may use the entropy source. */
        #[cfg(feature = "fips")]
        {
